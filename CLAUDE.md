...
- **審査が下りるまで、アカウントを非公開にしないと一切投稿できない。**
  2026-08-31 に実測: `PUBLIC_TO_EVERYONE` でも `SELF_ONLY` でも `video/init/` が
  HTTP 403（`unaudited_client_can_only_post_to_private_accounts`）で拒否した。
  エラーが指すのは投稿の公開範囲ではなく **TikTok アカウントの設定**
  （"All user accounts using the API client to post must be set to private at
  the time of posting."）。**`privacy_level_options` を審査状態の指標に
  使わないこと** — 未審査でも3つ返る。
...
2026-09-09 に追加。**全動画（ショート・長尺・TikTok）のナレーションの下に
自作曲を1曲固定で敷く。** 曲は `assets/bgm/petrichor.m4a`（`bgm-youtube` の
`2026-08-21-petrichor` から5分を切り出したもの）。

- **関門は `audio_mix.mix()` の1つ。** `build_short.build` と `build_long.build`
  の2箇所だけが通る（TikTok は `build_short.build` を再利用しているのでこれで全経路）。
  **呼び出し側で ffmpeg の音声入力を組み立てない。**
- **`audio_mix.MIN_SEPARATION_LU`（15 LU）を下げない。** ナレーションが BGM に
  埋もれた動画は成立しないので、ここだけは警告ではなく**例外で止めてある**
  （尺のズレ `verify_duration` は警告で通しているのと対照的）。
  設計値は 18 LU 下（`BGM_BELOW_VOICE_LU`）。
- **判定は書き出したファイルの実測で行う。** 計画値どうしを比べると差は定義上
  いつも 18 LU になり、フィルタの書き間違い・曲の取り違え・ゲインの掛け忘れが
  全部すり抜ける。
- **ebur128 の出力はサマリ部だけを読む。** 測定中のフレーム行にも `I:` が出て
  おり、stderr 全体を検索すると t=0.5秒時点の `-70.0`（＝無音）に一致する。
  実装当初これで全音源が無音判定になった。
- **Content ID の申し立てが自動で付く。** この曲は TuneCore で配信済みで、
  YouTube 収益化サービスにより Content ID に登録されている。動画の公開は
  止まらないが、**広告収益は自チャンネルではなく TuneCore 側の収益フローに
  流れる**（`bgm-youtube/docs/feasibility.md`）。**これは承知のうえの判断**
  （権利収入として戻る）。曲を差し替えるときも同じ前提を確認すること。
  Content ID を避けたい場合は、TuneCore に未提出の曲を使う。
- **TikTok での扱いは未検証。** TikTok は Content ID の外にあり、配信済み楽曲を
  投稿動画で使ったときにどうなるかは実測していない。
- ナレーションは元が -24.5 LUFS と静かなので、最終ミックスを **-14 LUFS**
  （YouTube の基準）まで持ち上げてリミッタでピークを -1.0 dBFS に抑えている。
  **BGM 導入前より音は大きくなっている。**
