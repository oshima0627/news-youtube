...
タイトル（検索向け）:
- 一次資料に事実があるときだけ、タイトルは「制度名＋施行時期＋影響を受ける人」の形にする。
- 施行日や対象者が引用に無いなら、タイトルに書かない（推測で補わない）。
- 答弁の列挙や公約の読み上げだけに見える題名・見出しは避ける。

ナレーションの冒頭:
- 最初の1文で結論を言う（制度・金額・施行日・誰が影響を受けるかのうち、引用にあるもの）。
- 発言者名・委員会名の紹介は、その結論の後に置く。前置きから入らない。
"""
...
class Script(BaseModel):
    title: str = Field(description="YouTubeのタイトル。60文字以内。ハッシュタグは含めない。一次資料に事実があるときだけ制度名＋施行時期＋影響を受ける人の形。無い事実は書かない")
    headline: str = Field(description="画面上部に出す見出し。20文字以内")
    narration: str = Field(description=f"読み上げる本文。{NARRATION_SPAN}。冒頭1文で結論（制度・金額・施行日・影響を受ける人のうち引用にあるもの）。発言者紹介はその後")
    subtitle: str = Field(
...
class LongMeta(BaseModel):
    ...
    title: str = Field(description="YouTubeのタイトル。60文字以内。ハッシュタグは含めない。可能なら制度名＋施行時期＋影響を受ける人。引用に無い事実は書かない")
...
def build_segment_prompt(recipe: dict) -> str:
    ...
    parts += [
        ...
        "ナレーションを書いてください。",
        "ナレーションの最初の1文で結論（制度・金額・施行日・誰が影響）を言い、"
        "発言者や委員会の紹介はその後に置いてください。",
        "この章の前後には別の題材の章が入ります。"
...
def build_prompt(recipe: dict) -> str:
    ...
    parts += [
        ...
        "書いてください。",
        "title は、一次資料に事実があるときだけ『制度名＋施行時期＋影響を受ける人』"
        "の形にしてください。引用に無い施行日・対象者は足さないでください。",
        "ナレーションの最初の1文で結論（制度・金額・施行日・誰が影響）を言い、"
        "発言者や委員会の紹介はその後に置いてください。",
        "subtitle には、字幕バンドに出す要点を40文字以内で書いてください"
        "（ナレーション全文ではありません）。",
