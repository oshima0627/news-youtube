...
最終更新: 2026-09-09（**この日に3つの作業が走った**。
① 動画のBGMに自作曲を入れた（`main` 済み）
② 24時間ライブ配信の設計と実装計画
③ **実装（Task 2-8, 10 完了）**。Task 1 と 9 は**公開配信を伴うので未実行**）
...
- **`work/live/loop.mp4` を実素材で焼き切れていない。** 57件ぶんの音声合成に
  時間がかかり、1度目は失敗（ログをパイプで捨ててしまい原因不明）、
  2度目を実行中。**「1周およそ40分」はまだ計算値で、実測していない。**
...
- **チャンネルでライブ配信が有効化されているか。** `liveBroadcasts.list` は
  API としては通ったが、1本流すまで分からない。
...
1. **loop.mp4 が焼き上がったか見て、尺を測る。**

   ```bash
   ffprobe -v error -show_entries format=duration -of csv=p=0 work/live/loop.mp4
   ```

   2,000〜2,900秒（約33〜48分）に入っていること。外れたら設計の前提が崩れる。

2. **Task 1: 前提の実測。** ⚠ **公開ライブ配信を伴う。オーナーの承認が要る。**
   短い公開配信を1本流して終了し、(a) アーカイブが残るか
   (b) **3日後以降**に `videoOnDemand` の再生時間が増えるか。現在 0.1時間なので
   増えれば一目で分かる。**増えなければこの機能は破棄する。**

3. **Task 9: 実配信で未検証3点を潰す。** ⚠ **これも公開配信。承認が要る。**
