...
最終更新: 2026-09-09（セッション: **24時間ライブ配信の設計**。収益化ルートを実測で比較し、
設計書を1本コミットした。**コードはまだ1行も書いていない**）
...
4. **設計書を新規作成してコミット**（`8ab06fa`）。自己レビューで2つの矛盾を直した
   （ffmpeg再起動とループ差し替えの両立／アーカイブ本数 730→760）。

実行したコマンド（スクラッチパッドの使い捨てスクリプト。リポジトリには入れていない）:

```bash
# channels.list + Analytics reports.query（creatorContentType 別）
python "$SD/wt.py"     # 直近365日
python "$SD/wt90.py"   # 直近90日
python "$SD/live.py"   # liveBroadcasts.list の疎通確認
```
...
- **`My5mRImj744`（相続税・9/8 07:30予約）と 9/7〜9/11 の選挙動画5本が実際に公開されたか。**
  今回のセッションでは確認していない。
- TikTok の審査状況。
...
3. **予約した動画が公開されたか見る。** 事故があれば
   `python scripts/unpublish.py <video_id>`。
4. **`claude/okinawa-governor-election-videos-d1ebd9` を統合する**（下記 ⚠）。
...
## ⚠ state がブランチ間で分裂している（未解決・前回から継続）

**9/7〜9/11 の選挙動画5本が、このブランチの `state/published.json` に無い。**
...
- **採用ゲートをライブ経路に再実装しない。** 入力は `recipes/*.json` の出力だけ。
  判定基準を2箇所に置くと必ず食い違う（known-issues 2・3・8番と同型）。
...
- **12時間を超える配信をしない。** 超えた瞬間アーカイブが作られず、その配信の再生時間は
  1分も4,000時間に入らない。関門は `run_live.py` の枠を作る関数の中に置く（呼び出し側に置かない）。
- **投票日は選挙関連を外したループに差し替える。** 動き続けるライブ配信は継続的な公開行為で、
  公職選挙法129条に触れる。
...
- **タイトル・説明文は日本語。** 一度英語にして戻した経緯がある（2026-09-07）。
  もう一度やるなら更新前のメタデータを必ず控えてから着手する。
- **英語化を画面（テロップ・引用カード）と音声に広げない。** `ensure_grounded_card` の検証が成立しなくなる。
