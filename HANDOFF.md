...
**✅ 9/11 の 07:30 は埋めた**（2026-09-10）。決めた手順の初回の適用。

```
3lmAbM8yS00  private → 2026-09-11 07:30 JST に自動公開（YouTube API で確認済み）
130万円の壁で就業調整 高市首相「働き控えにつながる」と答弁
一次資料: 第219回国会 衆議院予算委員会 2025-12-10 高市早苗
          https://kokkai.ndl.go.jp/txt/121905261X00720251210/116
```

**題材は `--keyword "年収の壁 就業調整 働き控え"` で国会会議録を直接掘って選んだ**
（RSS 候補は使っていない）。選んだ理由は Analytics の検索語実測で
「106万の壁 撤廃」220 /「年収の壁」18 など、この層の需要が確認できていること。

**これがチャンネルで最初の BGM 入り動画になる。** 実測値:
ナレーション -24.7 LUFS / BGM -42.7 LUFS / 分離 18.0 LU / 完成 -14.0 LUFS
/ ピーク -1.0 dBFS、尺 58.40秒、テロップ20枚。
**公開後、維持率が BGM 導入前と比べてどう動くかを見ること**（n=1 では判定しない）。
