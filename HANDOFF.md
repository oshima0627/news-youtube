...
- 予約済み枠は 8/27 07:30 まで連続で埋まっている。次の空きは 8/27 18:30。
...
1. 公開時刻前に、予約済み動画の見出しと引用の噛み合いを人が見る（README の運用ルール）。
   8/27 07:30 までの11本が対象。
2. 未予約の在庫: `8ebRhoO41tM`（ベルギー外交160周年、private・予約なし）は
   空き枠埋め（次は 8/27 18:30）に使える。`zC8OJmUT9Us`（憲法審査会）は
   噛み合い不良で外した経緯があるので使わない（known-issues 5-b番）。
