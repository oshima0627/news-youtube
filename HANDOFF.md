...
実 API での認証に成功 → 投稿を試して403で拒否され、**原因はアカウントの公開設定だと判明**。
本人の判断待ち: TikTok アカウントを一時的に非公開にするか）
...
**Sandbox の認証は完了。実 API に到達済み。** ただし審査前は
**アカウントを非公開にしないと一切投稿できない**ことが実測で判明した。
次の一手は本人がアカウントを非公開に切り替えること（下記「次にやること」1）。
...
- **公開でも SELF_ONLY でも 403 で拒否されることを実測した**（2回実行、
  log_id を含む同じエラー）。**審査前はアカウントを非公開にしない限り投稿できない。**
...
| 項目 | 状況 |
|---|---|
| アカウントの公開設定 | **本人の判断待ち。** 一時的に非公開にしないと投稿できない |
| 投稿経路の残り | 動画の PUT 送信と `status/fetch` のポーリングが未実行 |
| デモ動画（Production） | **未着手。** 上の投稿を録画すればそれが実演になる |

**循環している**: 審査にはデモ動画が要る → デモ動画には投稿の実演が要る →
投稿にはアカウントの非公開設定が要る。**非公開にしない限り TikTok 投稿は
実現できない。**
...
- **アカウントを非公開にすれば投稿が通るかは未検証。** ガイドラインの記述に
  基づく推測であって、実際に試していない。
...
1. **本人が TikTok アカウントを一時的に非公開にする**（設定 → プライバシー →
   非公開アカウント）。**これをしない限り投稿は 403 で通らない。**
   影響: 既存の投稿がフォロワー以外から見えなくなる／新規フォローが承認制に
   なる／おすすめ・検索から外れる。**Claude はこの設定を代わりに変更しない。**

   非公開にしたら、投稿と録画を一度に済ませる:

   1. 画面録画を開始
   2. `python scripts/upload_tiktok.py work/1d04e9d8cd04/tiktok --allow-self-only`
   3. TikTok アプリで投稿された動画を確認するところまで録画
   4. 録画停止 → **アカウントを公開に戻す**
