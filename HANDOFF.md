...
**TikTok は審査へ提出済み（in review）。YouTube は 9/2 07:30 を予約済み、
9/2 18:30 の動画はビルド済みでアップロードの確認待ち**）
...
### 19. 9/2 07:30 の枠を予約した

```
✓ https://www.youtube.com/watch?v=8639Yg7Ner0  (private)
✓ 予約しました → 2026-09-02T07:30:00+09:00 に自動公開
```

API で引き直して確認: `publishAt=2026-09-01T22:30:00Z`（= 9/2 07:30 JST）/
`duration=PT59S` / `processing=succeeded` / `lang=ja/ja`。

### 20. 9/2 18:30 の枠の動画をビルドした（`work/f879dad3635a`）

題材は「両親で育休なら手取り10割」（参議院予算委員会 2026-04-27 三原じゅん子）。
現金給付3.2兆円・所得制限なしで高校生年代まで・育休は手取り10割相当、と
家計の数字が3つ入る。画像は**三原じゅん子本人**（内閣府 / CC BY 4.0）。

```
試行1: speedScale=0.994 → 実尺58.51秒  テロップ21枚
尺: voice.wav 58.51秒 → video.mp4 58.53秒（差 +0.03秒）
```

画面外に出たテロップは0行。**アップロードは本人の確認待ち。**

### 21. 見出しに `normalize_numerals` が掛かっていないことを突き止めた

`normalize_numerals` は**引用カード（`cards.py:189`）とテロップ
（`telop.py:176`）にだけ**適用され、`render_headline` は呼んでいない。
そのため同じ画面で「育休は手取り十割」（見出し）と「手取り10割」（カード）
のように表記が割れる。

今回は**台本の見出しを数字で書き直して回避**した（描画の共通部分を変えると
全動画に影響するので触っていない）。**当面は見出しを算用数字で書くこと。**

### 22. 審査に要る3ページを Cloudflare Workers で公開した
...
| 9/2 07:30 の枠 | **予約済み**（`8639Yg7Ner0`） |
| 9/2 18:30 の枠 | **本人の確認待ち。** `work/f879dad3635a` をアップロードして予約するか |
...
1. **9/2 18:30 の枠にアップロードする**（本人の確認待ち）:

   ```bash
   python scripts/upload_youtube.py work/f879dad3635a
   python scripts/upload_youtube.py work/f879dad3635a --schedule 2026-09-02T18:30:00+09:00
   ```

   9/3 以降の枠はまだ空。同じ手順で埋める。題材は維持率の仮説
   （家計に直結する金額）で選ぶ。**見出しは算用数字で書く**（上記21）。
   未使用の候補: `高校 授業料 無償化`（ただし打越さく良の発言は「報道によれば」
   の伝聞で、外国人負担増にも触れるので扱いに注意）、`医療費 窓口 負担`。
...
- **見出し（`headline` / `title`）は算用数字で書く。** `normalize_numerals` が
  見出しには掛からないので、漢数字のまま出て引用カードと表記が割れる。
